  ...
  const blogPostSchema = z.object({
    ...
    featuredImage: z.instanceof(File).refine(file => file.size <= 5_000_000, "File size should be less than 5MB").optional(),
      ...
      <div className="max-w-4xl mx-auto p-6 bg-[#403e3e] my-8 rounded-md">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block mb-2 text-sm font-medium">Post Status</label>
              <select
                {...register("status")}
                className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-black"
              >
          ...
          {/* Editor Section */}
          <div>
            <label className="block mb-2 text-sm font-medium">Travel Story</label>
            <div className="border rounded-lg overflow-hidden">
              <div className="border-b bg-gray-50 flex gap-2 flex-wrap p-2">
                {/* Controls */}
                <ToolbarButton 
                  label="H2" 
                ...
                <div className="relative flex items-center gap-1">
                  ...
                  />
                  <div className="w-6 h-6 border rounded" style={{ backgroundColor: selectedColor }} />
                  <button
                    type="button"
                    className="p-2 rounded hover:bg-gray-100 text-black"
                    onClick={() => {
                ...
                />
                <label htmlFor="editorImage" className="p-2 rounded hover:bg-gray-100 text-black cursor-pointer">
                  Insert Image
                </label>
              </div>
  ...
  function ToolbarButton({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
    return (
      <button
        type="button"
        onClick={onClick}
        className={`p-2 rounded text-black ${active ? "bg-blue-100 text-blue-600" : "hover:bg-gray-100"}`}
      >
        {label}
      </button>
    )
